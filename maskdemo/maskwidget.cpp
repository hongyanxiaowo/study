...
void MaskWidget::showEvent(QShowEvent *)
{
	if (mainWidget != 0)
	{
		qDebug() << mainWidget->pos().x();
		qDebug() << mainWidget->pos().y();

		QPoint pt1 = mainWidget->mapFromParent(mainWidget->pos());
		//pt1 = mainWidget->pos();
		qDebug() << pt1.x();
		qDebug() << pt1.y();
		QPoint pt2 = mainWidget->mapToGlobal(pt1);
		qDebug() << pt2.x();
		qDebug() << pt2.y();
		this->setGeometry(pt2.x(), pt2.y(), mainWidget->width(), mainWidget->height());
	}
}
